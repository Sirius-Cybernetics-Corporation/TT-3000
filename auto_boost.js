          ...
          request(url_game_data, function(err, resp, html2)
            {
                if (!err)
                {
                 //log the data from the request
                  ...
                  var json_string = JSON.stringify(obj2);
                  // cleanup by removing quotes and brackets, might not be needed. 
                  var json_string_2 = json_string.replace(/['"]+/g, '');
                  var json_string_3 = json_string_2.replace(/[{}]+/g, '');
                  //pull champion id's followed by 3 digits and store into an array
                  let all_champ_ids = json_string_3.match(/championId:\d\d?\d?/g);
                  //pull the first 5 champs (your team only)
                  var only_tryhards = all_champ_ids.slice(0,5);
                  //select a random champ
                  random_champ = only_tryhards[Math.floor(Math.random()*only_tryhards.length)];
                  //keep only the champion number (remove "ChampionID" text)
                  random_champ_id = random_champ.replace(/\D/g,'');
                  console.log("\nRandom championId from team: ")
                  console.log(random_champ_id);
